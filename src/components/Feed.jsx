		...
		const fetchAllFollowing = async () => {
			try {
				const response = await fetch(
					`https://skaterodeo.com/api/follow/following/${userId}`
				);
				if (response.ok) {
		...
		const fetchAllFollowingClips = async () => {
			try {
				const clips = following.map(async (follow) => {
					const response = await fetch(
						`https://skaterodeo.com/api/clip/user/${follow.following.userId}`
					);
					if (response.ok) {
					...
					({
						...
						<div key={clipId} className="w-full p-4">
							<div className="border-2 rounded-2xl border-secondary p-4">
								<Link
									to={`https://skaterodeo.com/profile/${userProfile.userId}`}
								>
									{" "}
